...
```jsonc
{
  "schema": "tiers/v1",              // version gate, migrate on load
  "title": "LitRPG — 59 series, 3,044 hours",
  "tiers": [
    { "id": "s-plus", "label": "S+", "color": "#ff7f7f", "countsInRank": true }
  ],
  "items": [
    {
...
- **The editor board and the exported page are produced by the same `boardHtml()`.** The editor
  injects `BOARD_CSS` at runtime rather than keeping a parallel stylesheet, so the two cannot
  drift. What you drag is what you publish.

No build step and no dependencies: the editor is ES modules served statically.

## 5. Why URL-state is demoted to secondary
...
silverweed's pattern), or a single self-contained HTML file. Compact URL state stays as an
optional path for small lists, capped and feature-detected, never the state of record.
